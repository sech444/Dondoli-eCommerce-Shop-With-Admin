...
export default function LandingModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-1 sm:px-2">
      {/* Made popup smaller with reduced max-width and padding, shorter on mobile */}
      <div className="bg-white rounded-2xl shadow-xl p-3 sm:p-4 w-full max-w-xs sm:max-w-sm md:max-w-md flex flex-col items-center relative overflow-y-auto max-h-[70dvh] sm:max-h-[85dvh]">
        {/* Close button: top right on both mobile and desktop */}
        <button
          onClick={onClose}
          className="absolute top-3 right-3 text-green-700 hover:text-green-700 text-xl font-bold z-10 w-8 h-8 rounded-full bg-gray-100 hover:bg-green-100 flex items-center justify-center transition-colors duration-200"
          aria-label="Close"
        >
          &times;
        </button>
       ...
       <Image
          ...
          width={50}
          height={50}
          className="rounded-lg shadow-lg mb-2 sm:mb-3 w-16 h-16 sm:w-24 sm:h-24 object-cover mt-2 sm:mt-0"
          priority
        />
        ...
        <div className="flex flex-col items-center gap-2 sm:gap-3 w-full"> 
          ...
          <span className="text-gray-600 text-xs sm:text-xs">Fast, direct order via WhatsApp</span>
        </div>
