...
import AddToCartSingleProductBtn from "./AddToCartSingleProductBtn"; // Import the AddToCartSingleProductBtn component

// Define the props interface for ProductItem
interface ProductItemProps {
  product: Product;
  color?: string; // 'color' is now optional as per your new code
}

const ProductItem = ({ product, color}: ProductItemProps) => {
  // Corrected: Destructure 'title' instead of 'name' as per Product type
  // Removed 'name' from destructuring as 'title' is the primary field from Prisma.
  const { id, title, price, originalPrice, description, mainImage, slug } = product;

  const hasDiscount = originalPrice && originalPrice > price;
  const discountPercentage = hasDiscount
    ? Math.round(((originalPrice - price) / originalPrice) * 100)
    : 0; // Initialize to 0 if no discount
  ...
  return (
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden max-w-xs w-full transition-transform duration-300 hover:scale-105 flex flex-col h-full">
      ...
      <div className="p-4 flex-grow flex flex-col">
        ...
        {/* Add to Cart Button */}
        {/* The button itself should wrap the AddToCartSingleProductBtn component,
            and the styling should be applied to the outer button. */}
        <button className={`w-full bg-green-600 text-white py-2 px-4 rounded-md hover:bg-green-700 transition-colors mt-2`}>
          <AddToCartSingleProductBtn product={product} quantityCount={1} />
        </button>
      </div>
    </div>
